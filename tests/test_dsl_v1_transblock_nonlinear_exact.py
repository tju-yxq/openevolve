...
def _official_transblock(module):
...
def _radial_output_scales(official):
    ...
    for output_slice, scale in official.ga.sep_act.dtp.slices_sqrt_k.values():
...
def test_v1_nonlinear_transblock_matches_official_27_parameters_forward_gradients_and_symmetries():
    root = Path(__file__).resolve().parents[2] / "equiformer"
    official_module, _source = _load_official_module(root, torch)
    torch.manual_seed(20260762)
    ...
    mapping = dict(EQUIFORMER_V1_TRANSBLOCK_NONLINEAR_PARAMETER_MAPPING)
    ...
    assert sum(parameter.numel() for parameter in dsl_parameters.values()) == 736
