...
|位置|仓库或分支|版本说明|
|---|---|---|
|本地仓库|`C:\Users\26517\Documents\中关村\equivariant-nas`|`codex/equivariant-dsl`包含正式V1冻结功能基线`3744bdd`|
|GitHub远程仓库|[tju-yxq/openevolve的codex/equivariant-dsl分支](https://github.com/tju-yxq/openevolve/tree/codex/equivariant-dsl)|已推送正式V1冻结功能基线`3744bdd`；后续只允许追加文档和最终报告提交|
|A100服务器仓库|`/home/20262202788/equivariant-nas`|正式Run工作树固定为`3744bdd`，训练期间不切换Commit；后续文档提交只导入仓库引用，待实验结束后统一Fast-forward|
...
- 正式V1冻结功能基线结果：`169 passed, 6 skipped`；
- 跳过原因：Windows本地环境没有e3nn、timm和官方GPU运行依赖。

### 8.2A100服务器测试

- 正式V1冻结功能基线结果：`179 passed, 1 skipped`；
...
截至2026年7月26日的运行审计快照，8000-step阶段已经完成8个合法候选，四个因子均为2个候选，全部存在Checkpoint并保持`test_evaluated=false`。按终点Validation MAE排序的Top 4为：

|排名|候选ID|因子|8000-step Validation MAE|
|---:|---|---|---:|
|1|`507b05538a084e39`|`F4.4`|0.5719164532|
|2|`f83508221a0d7f40`|`F2.2`|0.6057192947|
|3|`399f8c8c516b2727`|`F5.3`|0.6060849239|
|4|`29ed11e129c7386b`|`F2.2`|0.6666767292|

控制器已经进入`promote_80k`阶段。第一个候选`507b05538a084e39`已从原8000-step Checkpoint恢复并完成80000 steps，结果已经原子写入`candidates_80000`：

|项目|结果|
|---|---:|
|80k终点Validation MAE|0.1755054943|
|轨迹最佳Validation MAE|0.1605232349|
|轨迹最佳step|77310|
|训练后最大相对等变误差|0.0006041417|
|训练后最大绝对等变误差|0.0052123070|

晋级排序必须使用80k终点Validation MAE`0.1755054943`，不能使用轨迹最佳值`0.1605232349`替代。该候选的Program ID为`507b05538a084e39`，Executable ID为`28f7213608f52b0aceb5`，80k Protocol ID为`e0664169c2`；Result和Checkpoint均存在，`valid=true`且`test_evaluated=false`。

控制器随后自动切换到第二个Top 4候选`f83508221a0d7f40`。它已从自己的8000-step Checkpoint恢复至global step 8000，并在相同固定1/4训练集上进入真实训练。搜索期间仍未访问Test，最终Test只能在Top 2、父代对照和Validation赢家冻结后由Finalizer以Evaluation-only方式执行一次。
