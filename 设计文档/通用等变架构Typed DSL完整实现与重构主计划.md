...
> 当前稳定运行时代码 HEAD：`aaeee1cef4636dc37f582132c7792d1c305fa323`；第一轮证据提交：`f6b83c8`
...
```text
分支：codex/v3-mapping
稳定运行时代码 HEAD：aaeee1cef4636dc37f582132c7792d1c305fa323（完成 V3 第一轮十轮候选生成入口）
第一轮证据提交：f6b83c8（记录 V3 十轮进化与检查点证据）
...
阶段提交五：af3be1f220b00b4531faa78554c21fbfcbe1717d（验证 V3 Energy Head 的官方数值对齐并提供证据导出工具）
阶段提交六：294d94c（记录 V3 Energy Head 官方对齐证据并更新主计划）
阶段提交七：550353f（记录 V3 能量任务输出布局缺口与修正计划）
阶段提交八：192fed55a217c9280642e3ef250b00125c35e581（新增 carrier-scalar 布局和通用 typed 单位轴压缩 Lowering）
阶段提交九：d2e5f7c952428c1eb2bbf60ce29ac91223d6cc55（完成 V3 两层 Energy+Force 官方整网对齐）
阶段提交十：0c28e9fff44a9ebce5b19cff9e98aa4d34beaf4f（补充 V3 能量布局、共享 frame 与共享 final norm 数学合同证据）
...
阶段提交十八：f6b83c8（记录 V3 十轮进化与检查点证据）
...
Backend-neutral semantics：backend-neutral-v22
e3nn backend semantics：e3nn-graph-lowering-registry-v24
当前 registry：103 个版本化 core primitive 注册项，103 条逐注册项 Lowering rule，8 个 reference motif
...
| M3：共同 P0 原语与 Lowering v2 | 进行中 | 已有 explicit topology、affine geometry、ParameterContract、invariant/equivariant multihead、五类 TP 参数化、V1 随机正则和 V3 Attention 证据；新增 V3 所需 typed concat/product/degree scale、V3 frame rotation、SO2Linear、GraphSoftmax、head contraction、SO3Linear、edge-frame degree gate 与 carrier-scalar 单位轴压缩，当前执行注册表与 Lowering 为 103/103；已建立 48 规范族的 canonical search surface，并把生成、可信补全和兼容上下文分层；其余 P0、统一 canonical core 签名、一般 instruction connection mode、完整 SO(2) v2、旧参数合同与 backend 文件拆分未完成 |
| M4：Grid/S² 下拆 | 进行中 | `tests/test_dsl_v3_grid_primitives.py`、`tests/test_dsl_v3_official_ffn.py`、`tests/test_dsl_v3_official_direct_model.py`；GridTensorType、GridSpec、project/unproject/split/concat/product/activation/channel-linear/dropout 已加入通用 registry；官方 FFN、TransBlock 和两层完整 Direct Model 已完成参数/数值/梯度 oracle，12 层 direct model 前向/反向 smoke 已覆盖 Grid 路径并修复 invariant gate 广播；有限网格仍仅具经验等变认证，真实生产分辨率与随机路径验收未完成 |
...
| `0.29.0` | 2026-08-01 | 新增 V3 Energy Head 官方 oracle 与证据导出工具；8 个规范化 typed 节点显式表达 final merge norm、scalar selection、两层 ScalarFeedForwardNetwork、SiLU、零概率 dropout、node-to-graph sum 和 `avg_num_nodes` 缩放；6 个参数张量/33 参数完成同 seed 初始化、参数双射、前向 RNG、图能量前向、节点特征梯度和全部参数梯度官方对齐；V3 扩展定向回归 86 passed，全量回归 354 passed、3 skipped；稳定运行时代码提交为 `af3be1f220b00b4531faa78554c21fbfcbe1717d`，证据位于 `reports/dsl_m8_v3_energy_head_oracle_20260801/` | 静态存在 energy-head 程序和完整模型 smoke 不能替代该头自身的官方数值闭环；只有冻结 final norm、参数树、聚合归一化、RNG 和梯度，才能把剩余风险收敛到完整 Energy+Force 整网组合、随机路径、checkpoint、stress 与训练轨迹 | M3、M4、M8 |
| `0.29.1` | 2026-08-01 | 在启动完整 Energy+Force 整网 oracle 时发现任务输出布局缺口：官方 V3 将单通道图能量输出为 `[graph]`，当前 Energy Head/Direct model 仍保留 `[graph, 1]`。计划新增 `carrier_scalar` 表示布局和 completion-only 的通用 typed 单位轴压缩原语，要求显式验证单位轴大小、运行时 rank/shape、等变性保持与非法压缩负例；随后刷新 Energy/Direct-model 证据，再继续完整整网 oracle | 参数、前向数值和梯度相等不能掩盖任务 API 的 rank/shape 不一致；无损导入与通用 Lowering 必须把可观察输出布局也纳入类型和模型身份 | M2、M3、M4、M8 |
| `0.30.0` | 2026-08-01 | 实现 `carrier_scalar` 表示布局和 completion-only `core.squeeze_unit_axis@1`，以静态单位轴合同与运行时 `[carrier,1] -> [carrier]` 检查关闭 V3 能量任务输出 rank；语义升至 `evoequilang-24` / `backend-neutral-v22` / e3nn registry v24，注册表为 103/103，canonical search surface 为 48 族且新增原语不暴露给 LLM。Energy Head 更新为 9 节点并完成官方 `[graph]` 输出、参数、RNG、前向和梯度对齐。新增 186 节点两层 Direct Energy+Force 官方整网 oracle，116 个参数张量/4,482 参数双射，官方 `EquiformerV3_OC` 与通用 Lowering 的初始化 RNG、两图 Energy `[2]`、Force `[5,3]`、前向 RNG、位置梯度和全部参数梯度均对齐；实际模型不含官方整网、Block、Attention 或任务头构造器；V3 扩展定向回归 91 passed，全量回归 359 passed、3 skipped；稳定代码提交为 `d2e5f7c952428c1eb2bbf60ce29ac91223d6cc55` | 整网复刻必须同时闭合任务 API 布局、模块共享身份、跨层随机 frame、完整参数树、RNG、双任务前向和全部梯度；独立算子/Backbone/Head oracle 不能替代组合后的官方整网执行 | M2、M3、M4、M8 |
| `0.31.0` | 2026-08-01 | 完成 V3 两层 Direct Energy+Force 整网的非零随机路径 oracle：同时启用 `alpha_drop=0.13`、`attn_weights_drop=0.17`、`value_drop=0.19`、`drop_path_rate=0.23`、`proj_drop=0.29`、`ffn_drop=0.31`，190 个规范化节点在 train/eval 下完成初始化 RNG、前向 RNG、Energy、Force、位置梯度和全部 116 个参数梯度官方对齐。审计发现旧 Lowering 仅在 S² 网格乘积分支执行 `value_drop`，遗漏官方标量 SwiGLU 分支的先行 dropout；现已补齐双掩码及其顺序合同。新增随机证据导出模式和 `reports/dsl_m8_v3_stochastic_full_model_oracle_20260801/`；定向随机回归 24 passed，全量回归 361 passed、3 skipped；稳定运行时代码提交为 `255fcf6599fc2792ced41ba582567e5f8495fc1f` | 随机路径等价不仅要求概率相同，还要求每个掩码位置、共享粒度、调用顺序、train/eval 行为及全局 RNG 消耗一致；尤其“值 dropout”是一个配置字段但对应两个有序随机算子，不能用单个近似掩码代替 | M3、M4、M8 |
| `0.32.0` | 2026-08-01 | 建立 V3 QM9-alpha 第一版正式进化主链：固定父代一次生成 8 个架构 ID 唯一直接子代，8k/80k 使用同一固定 1/4 数据并按 Validation MAE 8→4 晋级，250k 切换完整训练集并 4→2，最终赢家成为下一周期父代，连续执行 10 周期；batch size 固定为 8，Test 全程隔离。新增协议/数据/子集/等变合同 SHA-256 门禁、8k→80k 完整状态恢复、80k→250k model-only 与优化器/LR 重启、起点和实际执行步数检查、实际 Lowering/runtime/executable 身份写入赢家 manifest。修复 V3 自动 epoch 间隔在 Trainer 中被提前拒绝的问题，以及置换等变审计未同步重编号边索引的问题；等变合同收敛为当前类型系统实际声明的 SO(3)+平移+节点置换，不再误报 O(3) 反射。控制器和 V3 定向回归 21 passed；真实 8k/80k/250k GPU 训练尚未执行 | 候选内部的 Router/Critic/Synthesizer 不是多保真训练阶段；正式实验必须把“同父代 8 个兄弟候选”“8→4→2 资源分配”“终点 Validation 晋级”和“250k 赢家继承”编码为不可绕过的状态机，并验证实际输入文件与恢复步数，而不能只在文档中约定 | M8、M10、M11 |
